...
class DockerWechatHook:
    ...
    def change_port(self):
        time.sleep(5)
        result = subprocess.run(['curl', '-X', 'POST', 'http://127.0.0.1:18888/api/?type=35', '-H', 'Content-Type: application/json', '-d', json.dumps({"path": "/comwechat/http/WeChatHook.exe", "version": "3.9.12.16"})], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
